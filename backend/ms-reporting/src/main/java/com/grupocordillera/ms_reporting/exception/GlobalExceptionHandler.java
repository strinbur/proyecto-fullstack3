...
import com.grupocordillera.ms_reporting.glitchtip.GlitchTipErrorReporter;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<?> handleRuntime(RuntimeException ex) {
        ...
        if (ex instanceof DataAccessException) {
            errorReporter.captureException(ex, "Error de acceso a datos en ms-reporting");
        } else {
            errorReporter.captureMessage("Error de negocio en ms-reporting: " + ex.getMessage(), SentryLevel.WARNING);
        }
    ...
    public ResponseEntity<?> handleFeign(FeignException ex) {
        ...
        if (status >= 500 || status == 502) {
            errorReporter.captureException(ex, "Fallo de microservicio downstream (status " + status + ") en ms-reporting");
        } else {
    ...
    public ResponseEntity<?> handleGeneral(Exception ex) {
        errorReporter.captureException(ex, "Excepcion no controlada en ms-reporting");
